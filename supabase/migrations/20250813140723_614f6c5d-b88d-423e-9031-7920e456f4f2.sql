-- Update jackpot timer to cap at 30 seconds instead of 60 seconds
...
BEGIN
  ...
  -- Handle timer logic with 30-second maximum cap
  SELECT COUNT(DISTINCT user_id) INTO v_current_players
  FROM public.jackpot_tickets
  WHERE game_id = p_game_id;
  
  -- Calculate maximum allowed timer end (30 seconds from first ticket)
  SELECT created_at + INTERVAL '30 seconds' INTO v_max_timer_end
  FROM public.jackpot_games
  WHERE id = p_game_id;
  
  IF v_current_players = 1 THEN
    -- First player: start 20-second timer (leaving room for extensions)
    v_timer_end_at := NOW() + INTERVAL '20 seconds';
    -- Make sure it doesn't exceed 30 seconds from game creation
  ...
  ELSIF v_is_new_player THEN
    -- New player joining: add 5 seconds to timer, but cap at 30 seconds total
    SELECT timer_end_at + INTERVAL '5 seconds' INTO v_timer_end_at
    FROM public.jackpot_games
    WHERE id = p_game_id;
    
    -- Cap at maximum 30 seconds from game creation
    IF v_timer_end_at > v_max_timer_end THEN
      v_timer_end_at := v_max_timer_end;
    END IF;
