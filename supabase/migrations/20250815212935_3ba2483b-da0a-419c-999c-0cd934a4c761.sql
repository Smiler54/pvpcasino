-- Clean up unused database features for better performance
...
WHERE created_at < NOW() - INTERVAL '30 days'
AND type NOT IN ('credit_purchase', 'withdrawal_request'); -- Keep financial records longer

-- Optimize database by running VACUUM and ANALYZE
VACUUM ANALYZE;

-- Add indexes for better performance on frequently queried tables
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_coinflip_games_status_created 
ON public.coinflip_games(status, created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jackpot_games_status_timer 
...
-- Enable automatic cleanup with a function for future use
