-- Fix timer logic: Timer should start only when second player joins
...
BEGIN
  ...
  -- FIXED LOGIC: Start timer only when we transition from 1 to 2 unique players
  -- This means timer starts when the SECOND player joins, not the first
