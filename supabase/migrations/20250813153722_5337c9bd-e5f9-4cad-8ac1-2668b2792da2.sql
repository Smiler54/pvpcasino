-- Fix current active jackpot game without timer
UPDATE public.jackpot_games 
SET timer_end_at = NOW() + INTERVAL '30 seconds'
WHERE status = 'active' AND timer_end_at IS NULL;

-- Update the buy_jackpot_tickets function to use 30 seconds as base timer
...
BEGIN
  ...
  IF v_current_players = 1 THEN
    -- First player: start 30-second timer (leaving room for extensions)
