-- Security fix: Implement secure financial data access controls (Fixed)
...
CREATE POLICY "Admins must use secure functions only" 
ON public.profiles 
FOR SELECT 
...
CREATE POLICY "Users and admins limited access to withdrawals" 
ON public.withdrawals 
FOR ALL 
USING (
  (auth.uid() = user_id) OR 
  (has_role(auth.uid(), 'admin'::app_role) AND status IN ('pending', 'processing'))
);
...
AS $function$
BEGIN
  -- Only return data for completed matches and only to participants
  RETURN QUERY
  SELECT 
    gm.id,
    gm.amount,
...
CREATE POLICY "Users can view safe match data only" 
...
USING (
  ...
  CASE 
    ...
    ELSE server_seed IS NULL AND salt IS NULL
  END
);