...
CREATE INDEX IF NOT EXISTS idx_profiles_user_id_balance ON public.profiles(user_id, balance);
CREATE INDEX IF NOT EXISTS idx_transactions_user_id_amount ON public.transactions(user_id, amount) WHERE ABS(amount) > 100;
CREATE INDEX IF NOT EXISTS idx_withdrawals_user_id_status ON public.withdrawals(user_id, status, requested_at);
