...
-- Fix all database functions to prevent search path attacks
...
-- CRITICAL: Secure the make_user_admin function - restrict to super admin only
...
DECLARE
  ...
  v_caller_role TEXT;
BEGIN
  -- Check if caller is already a super admin (must have existing admin role)
...
BEGIN
  ...
  -- Validate method-specific details (encrypt sensitive data)
  ...
  -- Create withdrawal request (details will be encrypted at rest)
