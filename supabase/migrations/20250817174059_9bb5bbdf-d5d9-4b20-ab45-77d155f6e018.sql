-- Fix the buy_jackpot_tickets function to always set timer_end_at when second player joins
...
BEGIN
  ...
  -- CRITICAL FIX: Timer starts when second unique player joins OR timer is missing
  timer_should_start := (current_unique_players = 1 AND NOT user_already_playing) 
                        OR (current_unique_players >= 1 AND v_timer_end_at IS NULL);
  ...
  -- CRITICAL FIX: Always set timer to exactly 60 seconds when conditions are met
  IF timer_should_start THEN
    -- Calculate exact timer end time for consistency across all platforms
    ...
    INSERT INTO public.admin_audit_log (
      ...
      p_user_id,
      'jackpot_timer_started_v3_critical_fix',
      json_build_object(
        'game_id', p_game_id,
        ...
        'timer_was_null', (v_timer_end_at IS NULL),
        'trigger_reason', CASE 
          WHEN v_timer_end_at IS NULL THEN 'missing_timer_fix'
          ELSE 'second_unique_player_joined'
        END
      )
    );
  END IF;
  ...
  RETURN json_build_object(
    ...
    'unique_players_before', current_unique_players,
    'user_already_playing', user_already_playing,
    'timer_was_missing', (v_timer_end_at IS NULL)
  );
