-- Fix timer calculation in buy_jackpot_tickets function
...
BEGIN
  ...
  IF v_current_players = 1 THEN
    -- First player: set timer to initial duration (60 seconds) from now
  ...
  ELSIF v_is_new_player AND v_current_timer IS NOT NULL THEN
    -- New player joining: add extension time (5 seconds), but respect maximum total time (90 seconds max)
