-- Fix the draw_jackpot_winner function to handle concurrent executions
...
BEGIN
  ...
  IF v_game_status != 'active' THEN
    RETURN json_build_object(
      'success', false, 
      'error', 'Game is not active or already completed'
    );
  END IF;
  ...
  -- Check if we actually updated a game (maybe another process already completed it)
  IF NOT FOUND THEN
    RETURN json_build_object(
      'success', false,
  ...
  -- Try to create a new active game, but handle the case where one already exists
  BEGIN
  ...
  EXCEPTION WHEN unique_violation THEN
    -- If a new active game already exists, just get its ID
    SELECT id INTO v_new_game_id
    FROM public.jackpot_games
    WHERE status = 'active'
